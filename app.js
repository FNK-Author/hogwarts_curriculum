(function () {
  ...
  function renderSchedule() {
    ...
    els.scheduleMeta.textContent = state.muggleMode ? 'Getarnter Wochenplan · ' + customEvents().length + ' eigene Termine' + (state.aliasMode ? ' · Alias aktiv' : '') : state.year + '. Schuljahrgang · Hauslehrkraft: ' + house.head + ' · Wahlpflicht: ' + countMinimumElectives() + '/' + data.electiveRules[state.year].minimum;
    ...
    data.timeSlots.forEach(function (slot) { cells.push('<div class="grid-cell time-cell ' + (slot.type === 'meal' && !state.muggleMode ? 'meal-time' : '') + '" role="rowheader"><strong>' + slot.label + '</strong><span>' + slotDisplayTime(slot) + '</span></div>'); data.days.forEach(function (day) { cells.push(renderScheduleCell(day, slot, grouped.get(cellKey(day.id, slot.id)) || [])); }); });
    ...
    renderTrips(); renderMaterials(); renderSpecialDates(); renderDisguisePanel(); restartWritingAnimations();
  }
  ...
  function renderDisguisePanel() {
    ...
    els.disguisePanel.innerHTML = '<div class="muggle-head"><div><p class="eyebrow">Tarnzauber für Muggeltechnik</p><h2>Muggelplan bearbeiten</h2></div><div class="muggle-toggle-row"><button class="toggle-button ' + (state.muggleMode ? 'is-active' : '') + '" type="button" data-muggle-toggle>' + (state.muggleMode ? 'Muggelmodus deaktivieren' : 'Muggelmodus aktivieren') + '</button><button class="toggle-button ' + (state.aliasMode ? 'is-active' : '') + '" type="button" data-alias-toggle>Alias verwenden</button></div></div><p class="form-note">Lege hier harmlose Muggelmodule an. Ohne Alias sieht man zum Beispiel Mathe, Herr Müller, Raum 15; mit Alias kann daraus Arithmantik, Professor Septima Vector und Runenkabinett werden.</p>' + renderMuggleTimes() + renderMuggleCreator() + renderMuggleModules(modules);
    if (els.disguiseButton) els.disguiseButton.classList.add('is-active');
  }

  function renderMuggleTimes() {
    return '<section class="muggle-editor-block"><h3>Zeitspalte ändern</h3><div class="muggle-time-grid">' + data.timeSlots.map(function (slot) {
      return '<label><span>' + escapeHtml(slot.label) + '</span><input data-custom-time="' + slot.id + '" value="' + escapeHtml((state.customTimes && state.customTimes[slot.id]) || slot.time) + '" placeholder="' + escapeHtml(slot.time) + '"></label>';
    }).join('') + '</div></section>';
  }

  function renderMuggleCreator() {
    return '<section class="muggle-editor-block"><h3>Neues Modul anlegen</h3><div class="muggle-form-grid"><input data-new-field="name" placeholder="Mathe"><input data-new-field="teacher" placeholder="Herr Müller"><input data-new-field="room" placeholder="Raum 15"><input data-new-field="nameAlias" placeholder="Arithmantik"><input data-new-field="teacherAlias" placeholder="Professor Septima Vector"><input data-new-field="roomAlias" placeholder="Runenkabinett"><label class="color-field"><span>Modulfarbe</span><input data-new-field="color" type="color" value="#4465A6"></label><div class="session-field"><span>Zeit-Tagesblöcke</span><div class="session-check-grid" data-new-sessions>' + sessionOptions([]) + '</div></div><button class="primary-button compact" type="button" data-add-module>Modul hinzufügen</button></div></section>';
  }

  function renderMuggleModules(modules) {
    if (!modules.length) return '<section class="muggle-editor-block"><h3>Angelegte Module</h3><p class="empty-note">Noch keine Muggelmodule angelegt.</p></section>';
    return '<section class="muggle-editor-block"><h3>Angelegte Module</h3><div class="muggle-module-list">' + modules.map(function (module) {
      return '<article class="muggle-module-card" data-module-id="' + module.id + '" style="--accent:' + escapeHtml(module.color || '#4465A6') + '"><div class="muggle-module-title"><strong>' + escapeHtml(module.name || 'Unbenanntes Modul') + '</strong><button class="ghost-button compact" type="button" data-remove-module="' + module.id + '">Entfernen</button></div><div class="muggle-form-grid"><input data-module-field="name" value="' + escapeHtml(module.name || '') + '" placeholder="Name"><input data-module-field="teacher" value="' + escapeHtml(module.teacher || '') + '" placeholder="Lehrer"><input data-module-field="room" value="' + escapeHtml(module.room || '') + '" placeholder="Ort"><input data-module-field="nameAlias" value="' + escapeHtml(module.nameAlias || '') + '" placeholder="NameAlias"><input data-module-field="teacherAlias" value="' + escapeHtml(module.teacherAlias || '') + '" placeholder="LehrerAlias"><input data-module-field="roomAlias" value="' + escapeHtml(module.roomAlias || '') + '" placeholder="OrtAlias"><label class="color-field"><span>Farbe</span><input data-module-field="color" type="color" value="' + escapeHtml(module.color || '#4465A6') + '"></label><div class="session-field"><span>Zeit-Tagesblöcke</span><div class="session-check-grid" data-module-sessions>' + sessionOptions(module.sessions || []) + '</div></div></div></article>';
    }).join('') + '</div></section>';
  }
  ...
  function sessionOptions(selectedSessions) {
    ...
    return data.days.flatMap(function (day) {
      return data.timeSlots.map(function (slot) {
        ...
        return '<label class="session-check"><input type="checkbox" value="' + value + '" ' + (selected.has(value) ? 'checked' : '') + '><span>' + escapeHtml(day.label + ' · ' + slot.label + ' (' + slotDisplayTime(slot) + ')') + '</span></label>';
      });
    }).join('');
  }
  ...
  function handleDisguiseClick(event) {
    ...
    if (remove) { removeCustomModule(remove.dataset.removeModule); return; }
  }
  ...
  function addCustomModule() {
    ...
    var module = { id: 'custom-' + Date.now().toString(36), name: field('name') || 'Mathe', teacher: field('teacher') || 'Herr Müller', room: field('room') || 'Raum 15', nameAlias: field('nameAlias') || 'Arithmantik', teacherAlias: field('teacherAlias') || 'Professor Septima Vector', roomAlias: field('roomAlias') || 'Runenkabinett', color: field('color') || '#4465A6', sessions: selectedSessionsFromPicker(picker) };
  ...
  function customEvents() {
    return (state.customModules || []).flatMap(function (module, moduleIndex) {
      return (module.sessions || []).map(function (session, sessionIndex) {
        ...
        return eventFromItem({ id: module.id, name: name, color: module.color || '#4465A6', info: 'Dieses Modul wurde im Muggelplan manuell angelegt und kann in der Tarn-Konsole jederzeit geändert werden.', materials: [] }, 'custom:' + module.id + ':' + moduleIndex + ':' + sessionIndex, 'custom', session.day, session.slot, name, teacher, room);
      });
    });
  }
  ...
  function renderMobileDaySchedule(events) {
    ...
    var rows = data.timeSlots.map(function (slot) {
      ...
      return '<div class="mobile-day-row ' + (slot.type === 'meal' && !state.muggleMode ? 'mobile-meal-row' : '') + '"><div class="mobile-time"><strong>' + slot.label + '</strong><span>' + slotDisplayTime(slot) + '</span></div><div class="mobile-events">' + eventHtml + '</div></div>';
    }).join('');
  ...
  function renderTrips() {
    if (state.muggleMode) { els.tripSummary.innerHTML = '<p class="empty-note">Im Muggelmodus ausgeblendet.</p>'; return; }
  ...
  function renderMaterials() {
    if (state.muggleMode) { els.materialsTable.innerHTML = '<p class="empty-note">Im Muggelmodus werden keine Hogwarts-Materialien ausgegeben.</p>'; return; }
  ...
  function renderSpecialDates() {
    if (state.muggleMode) { els.specialDates.innerHTML = '<p class="empty-note">Im Muggelmodus ausgeblendet.</p>'; return; }
  ...
  function drawPdfSchedulePage(events, house) {
    ...
    ctx.textAlign = 'center'; ctx.fillStyle = '#111'; ctx.font = 'bold 28px Georgia'; ctx.fillText(state.name || 'Unbenannter Plan', 877, 70);
    ctx.font = '22px Georgia'; ctx.fillText(house.name + ' - ' + state.year + '. Schuljahrgang', 877, 106);
    ctx.font = '18px Georgia'; ctx.fillText(state.muggleMode ? 'Wochenübersicht' : 'Hogwarts Stundenplan', 877, 136);
    ...
    data.timeSlots.forEach(function (slot, r) {
      ...
      ctx.fillStyle = '#111'; ctx.font = 'bold 15px Georgia'; ctx.textAlign = 'left'; wrapText(ctx, slot.label, x0 + 8, y + 24, colW - 16, 16, 2); ctx.font = '12px Georgia'; wrapText(ctx, slotDisplayTime(slot), x0 + 8, y + 56, colW - 16, 12, 2);
  ...
  function slotTime(slotId) { var slot = data.timeSlots.find(function (item) { return item.id === slotId; }); return slot ? slot.label + ' ' + slotDisplayTime(slot) : slotId; }
  ...
  function labelForKind(kind) { return kind === 'club' ? 'Club' : kind === 'meal' ? 'Mahlzeit' : kind === 'custom' ? 'Muggelmodul' : 'Unterrichtsfach'; }
  ...
  function loadState() {
    var fallback = { name: '', house: 'gryffindor', year: 1, electives: [], clubs: [], trips: [], muted: false, scheduleFilter: 'all', mobileDay: 'monday', disguiseOpen: false, muggleMode: false, aliasMode: false, customTimes: {}, customModules: [] };
    try {
